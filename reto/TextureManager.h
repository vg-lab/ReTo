...
namespace reto {
	...
	class Texture2D: public Texture
	{
		public:
			...
			{
    			unsigned char *mpixels =new unsigned char[_width*_height*4];
    			//glReadPixels(0, 0, 800, 600, GL_RGBA, GL_UNSIGNED_BYTE, mpixels);
    			this->bind( );
				glGetTexImage(this->_target, this->_level, this->_format, this->_type, &mpixels[0]);
    			FIBITMAP* bitmap =FreeImage_Allocate(_width,_height,32,8,8,8);
    			for(int y =0 ;y<FreeImage_GetHeight(bitmap);y++)
			    {
			        BYTE *bits =FreeImage_GetScanLine(bitmap,y);
			        for(int x =0 ;x<FreeImage_GetWidth(bitmap);x++)
			        {
			            bits[0] =mpixels[(y*_width+x)*4+0];
			            bits[1] =mpixels[(y*_width+x)*4+1];
			            bits[2] =mpixels[(y*_width+x)*4+2];
			            bits[3] =255;
			            bits+=4;
			        }
			    }
