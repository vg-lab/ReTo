...
namespace reto
{
  ...
  void PerspectiveCamera::center( Eigen::Vector3f centeredPosition_ )
  {
    _pivot = Eigen::Vector3f( centeredPosition_(0),
                              centeredPosition_(1),
                              0.0f );
    _radius = -centeredPosition_(2)*10.0f;
    _Rotation( _RotationFromPY( 0.0f, 0.0f ) );
    _BuildViewMatrix();
    _BuildViewProjectionMatrix();
  }

  void PerspectiveCamera::zoom( float increment_ )
  {
    float scale = 1.0f;
    scale += increment_;
    _radius *= scale;
  ...
  void PerspectiveCamera::localTranslation( Eigen::Vector3f increment_ )
  {
  ...
  void PerspectiveCamera::localRotation( const float yaw_, const float pitch_ )
  {
    Camera::localRotation( yaw_, pitch_ );
    _BuildViewProjectionMatrix( );
  }

  bool PerspectiveCamera::anim( void )
  {
  ...
  float PerspectiveCamera::fov( void )
  {
    return _fov;
  }

  void PerspectiveCamera::rotation( const float yaw_, const float pitch_ )
  {
    if( !_isAniming )
    {
  ...
  Eigen::Vector3f PerspectiveCamera::pivot( void )
  {
    return _pivot;
  }

  void PerspectiveCamera::pivot( Eigen::Vector3f pivot_ )
  {
    if ( !_isAniming )
    {
      _pivot = pivot_;
  ...
  float PerspectiveCamera::radius( void )
  {
    return _radius;
  }

  void PerspectiveCamera::radius( const float radius_ )
  {
    if ( !_isAniming )
    {
      _radius = radius_;
...
#ifdef RETO_USE_ZEROEQ
  zeroeq::Subscriber* PerspectiveCamera::subscriber( void )
  {
    return _subscriber;
  }
#endif

  void PerspectiveCamera::targetPivot( Eigen::Vector3f targetPivot_ )
  {
    _isAniming = true;
    _firstStep = true;
  ...
  void PerspectiveCamera::targetRadius( const float targetRadius_ )
  {
    _isAniming = true;
    _firstStep = true;
  ...
  void PerspectiveCamera::targetPivotRadius(
      Eigen::Vector3f targetPivot_,
      float targetRadius_ )
  {
  ...
  void PerspectiveCamera::animDuration( const float animDuration_ )
  {
    _animDuration = animDuration_;
  }

  void PerspectiveCamera::_BuildViewMatrix( void )
  {
  ...
  void PerspectiveCamera::_BuildProjectionMatrix( void )
  {
    _projVec.resize( 16 );
  ...
  void PerspectiveCamera::_BuildViewProjectionMatrix( void )
  {
  ...
  void PerspectiveCamera::_PositionVectorized( const std::vector<float>& positionVec_ )
  {
    _positionVec = positionVec_;
  }

  void PerspectiveCamera::_Rotation( const Eigen::Matrix3f& rotation_ )
  {
    _rotation = rotation_;
  }

  void PerspectiveCamera::_ViewMatrixVectorized( const std::vector<float>& viewVec_ )
  {
    _viewVec = viewVec_;
  }
...
#ifdef RETO_USE_ZEROEQ
  void PerspectiveCamera::_OnCameraEvent( lexis::render::ConstLookOutPtr lookoutPtr_ )
  {
